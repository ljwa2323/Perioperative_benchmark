...
fill_last_values <- function(df, mask, col_list, time_col, var_dict) {
    ...
    # 设置列名
    colnames(result_df) <- c(time_col, col_list)
...
#' @return 无返回值，保存文件到指定路径
#' @import jsonlite
save_stats_to_json <- function(stats, file_path) {
  # 检查 stats 是否存在
  if (is.null(stats)) {
    stop("统计信息为空，请先运行 calculate_stats 方法")
  }
  ...
  tryCatch({
    ...
    stop(sprintf("保存统计信息到 %s 失败: %s", file_path, e$message))
  })
}
...
#' @return 统计信息列表
#' @import jsonlite
load_stats_from_json <- function(file_path, var_dict) {
  # 检查文件是否存在
  if (!file.exists(file_path)) {
    stop(sprintf("文件不存在: %s", file_path))
  }
  ...
  tryCatch({
    ...
    # 获取 var_dict 中的所有变量名
    var_dict_items <- unique(var_dict$itemid)
    # 获取 stats 中的所有变量名
    stats_items <- names(stats)
    
    # 检查是否有变量缺失
    missing_items <- setdiff(var_dict_items, stats_items)
    if (length(missing_items) > 0) {
      stop(sprintf("JSON 文件中缺少以下变量的统计信息: %s", 
                   paste(missing_items, collapse = ", ")))
    }
    
    # 检查是否有多余的变量
    extra_items <- setdiff(stats_items, var_dict_items)
    if (length(extra_items) > 0) {
      stop(sprintf("JSON 文件中包含 var_dict 中不存在的变量: %s", 
                   paste(extra_items, collapse = ", ")))
    }
    
    # 检查每个变量的类型是否匹配
    for (itemid in var_dict_items) {
      var_type <- var_dict$value_type[var_dict$itemid == itemid]
      if (stats[[itemid]]$type != var_type) {
        stop(sprintf("变量 %s 的类型不匹配: var_dict 中为 %s, JSON 文件中为 %s",
                     itemid, var_type, stats[[itemid]]$type))
      }
    }
...
#' @return z转换参数列表
csv_to_z_param <- function(csv_path) {
  # 检查文件是否存在
  if (!file.exists(csv_path)) {
    stop(sprintf("文件不存在: %s", csv_path))
  }
  
  # 读取 CSV 文件
  stats_df <- read.csv(csv_path)
  
  # 检查必要的列是否存在
  required_cols <- c("name", "mean", "sd")
  if (!all(required_cols %in% names(stats_df))) {
    stop("CSV 文件必须包含 'name', 'mean', 'sd' 列")
  }
  
  # 创建 z_param 列表
  z_param <- list()
  for (i in 1:nrow(stats_df)) {
    z_param[[stats_df$name[i]]] <- list(
      mean = as.numeric(stats_df$mean[i]),
      sd = as.numeric(stats_df$sd[i])
    )
  }
...
#' 将 z_param 列表保存为 CSV 文件
#' 
#' 将 z_param 列表结构转换为 CSV 文件，包含以下列：
#' - name: 变量名
#' - mean: 均值
#' - sd: 标准差
#' 
#' @param z_param z转换参数列表
#' @param csv_path CSV 文件的保存路径
#' @return 无返回值，保存文件到指定路径
z_param_to_csv <- function(z_param, csv_path) {
  # 检查 z_param 是否为空
  if (is.null(z_param) || length(z_param) == 0) {
    stop("z_param 为空")
  }
  ...
  stats_df <- data.frame(
    name = names(z_param),
    mean = sapply(z_param, function(x) x$mean),
    sd = sapply(z_param, function(x) x$sd)
  )
  ...
  tryCatch({
    ...
    stop(sprintf("保存 z_param 到 %s 失败: %s", csv_path, e$message))
  })
}
